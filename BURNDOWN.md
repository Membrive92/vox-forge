...
| BAJO-29 | WorkbenchTab sobredimensionado | RESUELTO | 1745→883 (4ba952e) →545 (F4, objetivo <600 cumplido): `ProjectSidebar` (+fila), `ProjectVoicePicker`, `ProjectCoverPicker` a ficheros propios; ChapterCard 897→517 partido en `ChapterAudioPanel` (player+upload+grabación+TakeSelector con su estado local) + `ChapterVideoActions` (render/cancel con su abort) + `ChapterVoicePicker` — movimientos puros, cero cambio de comportamiento; e2e 10 passed/3 skipped sin tocar asserts — F4 |
| BAJO-30 | Prop drilling Voices (~17 props) | RESUELTO | `synthFormContext` (patrón profilesContext, provider en App): el estado del formulario (voz/formato/sliders/draft/dragOver/players de preview/texto con autosave) y los 5 handlers de perfil viven en `SynthFormProvider`; VoicesPlusLab y VoicesUnifiedTab pasan de 17/13 props a `{t, onToast}`; las hojas (VoicesTab/ProfilesTab/QuickSynthTab) siguen prop-driven — consumen, no taladran; `toggleLang` en el contexto porque cambiar idioma resetea la voz por defecto (puente `AppHeader`); e2e edit-profile/profile-sync verdes sin tocar asserts — F4 |
...
| PROD-01 | F7 | `POST /api/engines/unload` (política GPU inter-proceso) | RESUELTO — `routers/engines.py`: adquiere `gpu_semaphore` (nunca descarga a mitad de una inferencia; espera a GPU ociosa), descarga XTTS vía `TTSEngine.unload_clone_engine()` (público, no instancia el engine solo para descargarlo) + OpenVoice vía `ConvertEngine.unload_model()` y vacía la caché CUDA con guard de disponibilidad de torch; `EnginesUnloadResponse` (`unloaded: ["xtts","openvoice"]` solo con modelos realmente residentes + `cuda_cache_cleared`) con `response_model` y openapi/tipos regenerados; el docstring del router fija la política inter-proceso (llamar antes de cargas visuales pesadas — el provider de PROD-02 lo invocará); tests con engines stubbeados: reporta residentes, no-op sin modelos, no re-descarga un clone engine ya vacío, y bloquea mientras el semáforo GPU está ocupado — F7 `feat(prod) PROD-01` |
| PROD-02 | F7 | ComfyUIProvider real (stack Apache, títulos, settings) | RESUELTO — `ComfyUIProvider` en `services/image_gen.py`: HTTP puro contra la MISMA instancia/workspace de ComfyUI del `img_generation_module` (`POST /prompt` → poll `/history/{id}` → descarga `/view`; el backend nunca ejecuta difusión); parametrización por `_meta.title` (PROMPT_POSITIVE/SEED/SIZE/SAVE — convención ADR-005 espejada, sin importar del módulo; SEED multi-nodo con `seed`/`noise_seed`, negative del export se respeta); buckets %16 de Z-Image-Turbo (1248×720 16:9 canónico — cero modelos del plan viejo, stack Apache); settings `image_provider`/`comfyui_url`/`comfyui_timeout_s`/`comfyui_workflow_path` (default el export PROD-05; relativo resuelve contra base_dir) vía env `VOXFORGE_*`; workflow ausente/sin títulos ⇒ `ImageWorkflowError` 503 con el paso HUMANO completo en el mensaje (engine up → GUI → Save API Format → SETUP_PROVISIONING §5; los códigos nuevos quedan fuera de `_USER_FRIENDLY_MESSAGES` adrede para que el detalle accionable llegue al toast del ImageGenDialog); PROD-01 invocado in-process antes de cada job (`services/engine_unload.unload_gpu_engines`, extraído del router — nunca HTTP-to-self) ⇒ XTTS/OpenVoice descargados y caché CUDA vacía antes de la difusión; health check `GET /api/studio/image-provider/status` (workflow primero sin red, luego `/system_stats` con timeout 2 s) + el dialog hace probe al abrir y muestra aviso ámbar offline / nota solo si placeholder (i18n es/en); PNG resultante a `STUDIO_COVERS_DIR` por el mismo camino que el placeholder; openapi/tipos regenerados; README con sección de setup (cierra el placeholder de PROD-04); 20 tests con `httpx.MockTransport` (sin red): parametrización por título, buckets, happy path bytes+orden unload→submit, workflow ausente accionable, server caído 503 con instrucciones, 400 grafo inválido, timeout, execution_error, health ×4, factory, endpoint integración ×2. **NOTA: el E2E en modo real (imagen de verdad) queda gated por PROD-05 HUMANO — sin el export `zimage_t2i_fp8.api.json` el provider falla con la instrucción, que es el comportamiento diseñado.** — F7 `feat(prod) PROD-02` |
| PROD-03 | F7 | Detección de escenas desde SRT | RESUELTO — `services/scene_detect.py` puro (`parse_srt` tolerante a bloques rotos/CRLF + `group_into_scenes`: acumula hasta `target_scene_seconds`, pausa ≥1.5 s cierra como párrafo, entrada más larga que el target es escena propia sin inflar vecinas; resumen = primeras 12 palabras); `POST /api/studio/scenes/detect` (`DetectScenesRequest` con `srt_path` confinado a allowed roots + `target_scene_seconds` 5–300 default 25, `response_model=DetectScenesResponse {scenes:[{start_s,end_s,text_summary}],count}`, sin persistencia: el cliente alimenta `images` de `/render-video`); openapi+tipos regenerados; FE: `detectScenes()` tipado en `src/api/studio.ts` SIN consumidor de UI por diseño (llega con UX-03/M5 — no podar); tests: 12 unit puros (SRT vacío, entrada única, larga > target, múltiplos exactos sin escena colgante, párrafo, gap pequeño, resumen) + 6 de router (agrupación default/custom, vacío ⇒ 200 lista vacía, 400 path fuera de roots, 404 falta SRT, 422 target fuera de rango) — F7 `feat(prod) PROD-03` |
| PROD-04 | F7 | Coherencia documental (SUPERSEDED + README) | RESUELTO — `production-workflow-plan.md`: cabeceras `> SUPERSEDED` en §B3 (sin APIs externas; sobrevive solo el endpoint + provider pattern ya implementado) y §B4 (sin diffusers embebido ni Ollama — ADR-001), ambas apuntando a `img_generation_module/docs/DECISIONS.md` y a PROD-02, más línea de Estado con lo ya implementado (A1–A3, C1–C3, B1) vs pendiente (B2=PROD-03, A4/C4/C5 backlog); `comfyui-integration-plan.md`: Estado aclara arquitectura-vigente/modelos-supersedidos y cabecera `> SUPERSEDED` en la tabla de modelos (JuggernautXL descartado, FLUX prohibido por licencia, stack canónico Z-Image-Turbo fp8 ADR-002/003, parametrización por `_meta.title` ADR-005, misma instancia ComfyUI del módulo); README: "local-first" matizado (Edge-TTS sintetiza en la nube de Microsoft; el resto 100% local), retirados de "Planned" los items verificados en código (slideshow+Ken Burns+waveform overlay en `video_renderer.py`, subtítulos faster-whisper en `/api/studio/transcribe`, LUFS/denoise/compresor en `audio_editor.VALID_OPERATIONS`, upload/grabación/multi-take en `chapter_synth.py` + `ChapterRecorder.tsx`) y promovidos a Features, nota placeholder de que la sección de setup de ComfyUI llega con PROD-02, contadores de tests actualizados (334/78) — F7 `docs(prod) PROD-04` |
...
| UX-01 | F7 | Job tray global no bloqueante (JobsContext) | RESUELTO — `jobsRegistry.ts` (reducer puro) + `JobsProvider` en App (contextos estado/acciones separados para que los feeders no re-rendericen con ticks ajenos) + `useJobMirror`: los hooks dueños siguen siendo dueños del polling/await y solo reflejan su estado vía efectos (registro al activarse, patch en vivo, complete al desactivarse o desmontarse; los completados persisten 4 s como "done" y se autolimpian). Feeders cableados: quick synth (`useSynthesis`, con progress/chunks reales), síntesis de capítulo (ChunkMap), conversión (ConvertTab), render de vídeo (useVideoRender en Studio + ChapterVideoActions en Workbench), transcripción (useTranscription) y clonación cross-lingual (ExperimentalTab). UI: barra fina agregada bajo TabsNav (media de los determinados; sweep indeterminado si ninguno reporta), tray en el Header (spinner+contador, dropdown con tipo/detalle/estado, click → pestaña de origen) y badge de contador en el botón de la pestaña dueña (patrón errorBadge; los jobs de Studio no tienen entrada de nav — el tray los cubre). El patrón `pendingStudioSourceId` generalizado a `navigate(NavigationIntent)` en App (`tab` / `studio-source` / `resume-jobs`); header a zIndex 20 para que el dropdown no quede bajo la nav. i18n es/en (13 claves `jobs*`/`jobKind*`). Tests: 9 unit del reducer, 4 del ciclo mirror/provider (linger con fake timers, unmount, sin provider), 4 de componente del tray (render+intent de navegación), y e2e `jobs-tray.spec.ts` (síntesis en Voices → Workbench → tray muestra el job → click vuelve; mock con delay para observarlo en vuelo) — 12 passed/3 skipped |
...
- **PROD-05**: `cd img_generation_module && python -m pipeline engine up` → exportar los 2 workflows (SETUP_PROVISIONING §5) → rellenar `docs/BENCHMARK.md`. Desbloquea también el modo real de PROD-02: el provider carga ese export (`VOXFORGE_IMAGE_PROVIDER=comfyui`) y hasta entonces falla con esta misma instrucción.
